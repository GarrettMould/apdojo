...
export default function ChangesADASShortRunDeepDivePage() {
  const lessonPills = [
    { label: 'AP Macro - 3.6' },
  ];

  const flashcards = getFlashcardsForLesson('macro', 3, '3.6');

  const fallbackVideo = useMemo(() => {
    const video = videos.find(
      v => v.unit === '3' && v.lessonIDS.includes('3.6') && v.subjects.includes('AP Macroeconomics')
    );
  ...
  const lessonMcqQuestions = useMemo(() => {
    const matching = allQuestions.filter(
      q => q.unit === 3 && q.subject === 'ap_macroeconomics' && q.lessonIDS?.includes('3.6')
    );
  ...
  return (
    <DrillDeepDive
      ...
      lessonTitle="Changes in the AD-AS Model in the Short Run"
