  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (err instanceof FirebaseError && (err.code === 'auth/cancelled-popup-request' || err.code === 'auth/popup-closed-by-user')) {
        ...
        setError('Sign up with Google failed. Please try again or create an account with email.')
      }
    } finally {
        ...
        <div className="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl border border-gray-200 p-8 md:p-10" style={{ boxShadow: '0 0 40px rgba(59, 130, 246, 0.3), 0 0 80px rgba(59, 130, 246, 0.15)' }}>
              ...
              <form className="space-y-6" onSubmit={handleSubmit}>
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 py-3 px-4 border-2 border-slate-300 rounded-xl text-slate-700 bg-white hover:bg-slate-50 font-semibold text-base focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-slate-400 disabled:opacity-50 transition-all"
                >
                  ...
                  {loadingGoogle ? 'Creating account...' : 'Continue with Google'}
                </button>
